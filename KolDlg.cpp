...
void CPageDlg::OnOK()
{
    if (theApp.swCZV == ToolbarMode::tryb_studia && m_prn_mak.GetCount()) {
        ...
        CManODPNETParms orapar {
            ...
            { CManODPNET::DbTypeInt32, &m_id_str },
            { CManODPNET::DbTypeInt32, &m_prn_mak_xx },
            { CManODPNET::DbTypeInt32, &prn_flag },
            { CManODPNET::DbTypeInt32, &m_topage }
        };
...
BEGIN_MESSAGE_MAP(CPageDlg, CDialog)
    ...
    ON_CBN_SELCHANGE(IDC_NAMECOMBO, GiveOutStrLog)
    ON_CBN_SELCHANGE(IDC_PRN_MAK, OnSelchangePrnMak)
...
void CPageDlg::SetFunListBox(BOOL setDefaults)
{
    ...
    CManODPNETParms orapar { CManODPNET::DbTypeInt32, &m_prn_mak_xx };
...
void CPageDlg::OnBnClickedAcdead()
{
    ...
    if (dlg.DoModal() == IDOK) {
        ...
        CManODPNETParms orapar {
            { CManODPNET::DbTypeInt32, &m_mak_xx },
            { CManODPNET::DbTypeInt32, &m_id_str },
            { CManODPNET::DbTypeVarchar2, &sDataRed },
            { CManODPNET::DbTypeVarchar2, &sDataFot },
            { CManODPNET::DbTypeVarchar2, &sDataKol }
        };
...
BOOL CPageDlg::OnInitDialog()
{
    ...
    if (theApp.swCZV == ToolbarMode::tryb_studia && theApp.isRDBMS && (theApp.grupa&(UserRole::stu | UserRole::mas))) {
        ...
        int prn_flag;
        CManODPNETParms orapar {
            { CManODPNET::DbTypeInt32, CManODPNET::ParameterOut, &prn_flag },
            { CManODPNET::DbTypeInt32, CManODPNET::ParameterOut, &m_prn_mak_xx },
            { CManODPNET::DbTypeInt32, &m_mak_xx },
            { CManODPNET::DbTypeInt32, &m_id_str }
        };
...
BEGIN_MESSAGE_MAP(CAddDlg, CDialog)
    ...
    ON_BN_CLICKED(IDATEX, OnAtex)
    ON_BN_CLICKED(IDC_CBPOWT, OnCbpowt)
    ON_BN_CLICKED(IDC_ODBLOKUJ, OnBnClickedOdblokuj)
    ON_NOTIFY(DTN_DATETIMECHANGE, IDC_POWT, OnDtnDatetimechangePowt)
...
void CAddDlg::OnEnChangeWersja()
{
    ...
    if (isZajwMode && m_zajawkacombo.GetCount() == 0) {
        ...
        CManODPNETParms orapar {
            { CManODPNET::DbTypeInt32, &theApp.activeDoc->m_mak_xx },
            { CManODPNET::DbTypeVarchar2, &sWersja },
...
void CAddDlg::OnEmisje()
{
    if (!m_fullView) {
        ...
        CManODPNETParms orapar { CManODPNET::DbTypeInt32, &add_xx };
...
void CAddDlg::OnBnClickedOdblokuj()
{
    if (theApp.isRDBMS && UpdateData()) {
        ...
        CManODPNETParms orapar {
            ...
            { CManODPNET::DbTypeVarchar2, &theApp.activeDoc->data },
            { CManODPNET::DbTypeInt32, &adno }
        };
...
void CAddDlg::OnEnUpdateOldadno()
{
    ...
    if (!m_candidateAdnoDate.IsEmpty()) { //odczytaj kandyduj¹ce do powtórki numery adno
        CManODPNETParms orapar {
            { CManODPNET::DbTypeVarchar2, &m_candidateAdnoDate },
            { CManODPNET::DbTypeInt32, &m_pub_xx },
            { CManODPNET::DbTypeRefCursor, CManODPNET::ParameterOut, nullptr }
        };
...
BEGIN_MESSAGE_MAP(CDrzDlg, CDialog)
    //{{AFX_MSG_MAP(CDrzDlg)
    ON_NOTIFY(DTN_DATETIMECHANGE, IDC_LASTEMISION, OnKiedyChanged)
    ...
    if (isON) {
        ...
        if (iIleDni > 1 || IsDlgButtonChecked(IDC_MEMSTAT)) {
            ...
            CManODPNETParms orapar2 {
                ...
                { CManODPNET::DbTypeVarchar2, &mut },
                { CManODPNET::DbTypeVarchar2, &kiedy },
                { CManODPNET::DbTypeInt32, &iIleDni },
                { CManODPNET::DbTypeInt32, &isGrb },
                { CManODPNET::DbTypeRefCursor, CManODPNET::ParameterOut, nullptr },
                { CManODPNET::DbTypeRefCursor, CManODPNET::ParameterOut, nullptr }
            };
...
BEGIN_MESSAGE_MAP(CDBOpenDlg, CDialog)
    ...
    ON_EN_CHANGE(ID_TYTUL, OnChangeTytul)
    ON_EN_CHANGE(ID_MUTACJA, OnChangeMutacja)
    ON_BN_CLICKED(IDC_LIB, RefreshCombo)
    ON_BN_CLICKED(IDC_GRZBIET, RefreshCombo)
    ON_BN_CLICKED(IDC_MAKIETA, RefreshCombo)
    ON_BN_CLICKED(IDC_MEMSTAT, &CDBOpenDlg::OnBnClickedAllMut)
    ON_CBN_SELCHANGE(IDC_OKRES, RefreshCombo)
    ON_NOTIFY(DTN_DATETIMECHANGE, IDC_DT, OnDtnDatetimechangeDt)
    //}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CPageDerv::OnInitDialog()
{
    ...
    m_base_nr = m_nr;
    CManODPNETParms orapar {
        { CManODPNET::DbTypeInt32, CManODPNET::ParameterInOut, &m_drw_xx },
        { CManODPNET::DbTypeInt32, CManODPNET::ParameterInOut, &m_base_nr },
        { CManODPNET::DbTypeRefCursor, CManODPNET::ParameterOut, nullptr }
    };
    orapar.outParamsCount = 2;
